  ...
  return (
    <div className="space-y-8 pb-20">
      <div>
        ...
        <p className="text-slate-500 font-mono text-sm mt-1">Chronological record of your progression.</p>
      </div>
      ...
      {loading ? (
        <div className="flex justify-center py-12">
         ...
         <div className="text-center py-16 border border-slate-800 border-dashed rounded-2xl text-slate-500 font-mono">
        ...
        <div className="relative border-l border-slate-800 ml-4 md:ml-8 space-y-8 pb-8">
          {history.map((record, index) => (
             ...
             >
                {/* Timeline Dot */}
                <div className={`absolute -left-3 md:-left-4 w-6 h-6 rounded-full border-4 border-slate-950 flex items-center justify-center ${record.action === 'ITEM_PURCHASED' ? 'bg-amber-400' : 'bg-amber-400'}`}>
                  {record.action === 'ITEM_PURCHASED' ? <ShoppingCart size={10} className="text-slate-900" /> : <Target size={10} className="text-slate-900" />}
                </div>
                
                <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 hover:border-amber-500/30 transition-colors">
                   ...
                   {record.action === 'QUEST_COMPLETED' && (
                     <>
                       <h3 className="text-lg font-bold text-white mb-3">
                         Completed: <span className="text-amber-400">{record.details?.questTitle || 'Unknown Quest'}</span>
                       </h3>
                       ...
                       {record.details?.levelUp && (
                         <div className="mt-4 inline-block px-3 py-1 border border-amber-400/50 bg-amber-400/10 text-amber-400 font-mono text-xs tracking-widest rounded-md">
