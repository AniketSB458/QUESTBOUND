      ...
      >
        ...
        <button
          ...
          onClick={async () => {
            ...
            setLoading(true);
            try {
              const { data } = await api.post('/auth/login', { email: 'demo@nexus.net', password: 'nexus2077' });
              login(data);
              navigate('/dashboard');
            } catch (err: any) {
          ...
          disabled={loading}
          className="w-full bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/50 text-amber-400 font-bold tracking-widest py-3 rounded-lg transition-all hover:shadow-[0_0_20px_rgba(251,191,36,0.2)] flex items-center justify-center font-mono"
        >
